...
def register_handlers(bot):
    ...
    def add_user_prompt(message):
        ...
        if message.chat.id in ADMINS:
            bot.send_message(message.chat.id, "Введите данные пользователя в формате: /add_user <ID> <Статус> <Дата окончания подписки (ГГГГ-ММ-ДД)>")
        else:
            bot.send_message(message.chat.id, "❌ У вас нет доступа к этой функции.")
    ...
    def add_user_command(message):
        ...
        if message.chat.id in ADMINS:
            try:
                ...
                bot.send_message(message.chat.id, "❌ Ошибка! Используйте формат: /add_user <ID> <Статус> <Дата окончания подписки (ГГГГ-ММ-ДД)>")
            except Exception as e:
